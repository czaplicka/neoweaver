	...
	private function get_core_context( string $char_id ): array {
		...
		if ( ! empty( $char['location_id'] ) ) {
			$location = $this->query(
				'cyber_world_map',
				[
					'id'     => 'eq.' . $this->sanitize_uuid( $char['location_id'] ),
		...
		if ( ! empty( $char['world_id'] ) ) {
			$world = $this->query(
				'cyber_worlds',
				[
					'id'     => 'eq.' . $this->sanitize_uuid( $char['world_id'] ),
