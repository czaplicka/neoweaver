...
/**
 * Helper: resolve Supabase storage URLs if needed.
 */
function nw_resolve_img_urls( array $rows ): array {
    foreach ( $rows as &$row ) {
        if ( ! empty( $row['img_url'] ) && is_string( $row['img_url'] ) ) {
            // Here you could prepend Supabase storage URL if needed.
            $row['img_url'] = esc_url_raw( $row['img_url'] );
        }
    }
    unset( $row );
    return $rows;
...
/**
 * Generic lookup-cache fetcher via Supabase.
 */
function nw_fetch_lookup_table(
    ...
    $cache_key = sprintf(
        'neoweaver_lookup_%s_%s_%s_%s',
        $table,
        md5( $select_cols ),
        md5( $order ),
        md5( wp_json_encode( $extra_filters ) )
    );
...
/**
 * Filter starting packages by class tag (class name as tag).
 */
...
/**
 * Find race by name (for older flows).
 */
...
/**
 * Find race by id (used for race/subrace UUID validation).
 */
...
/**
 * Validate starting package vs. class.
 */
function nw_validate_starting_package_selection( string $class_id, string $package_id ) {
    ...
    // Class tag = class name in lowercase.
...
/**
 * Find tag definitions by labels (for backstory tags).
 */
...
/**
 * Find class by id (used for skill_limit, active flag).
 */
...
/**
 * Find skills by ids (only active).
 */
...
/**
 * Validate skills vs. class skill_limit and active flags.
 */
...
/**
 * Validate backstory tags exist in defs.
 */
...
/**
 * Store character skills (junction).
 */
...
/**
 * Store character backstory tags (junction).
 */
...
/**
 * AJAX: create character from request.
 */
function nw_create_character_from_request() {
    ...
    // Race/subrace: prefer subrace if provided.
    ...
    $payload = [
        ...
        // narrative choices – możesz użyć ich później w kampanii
        'data_origin'       => $data_origin ?: null,
        'prev_operation'    => $prev_operation ?: null,
        'sync_crisis'       => $sync_crisis ?: null,
    ];
