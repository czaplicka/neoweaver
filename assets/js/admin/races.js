...
(function ($) {
	...
	/**
	 * Given a filename (e.g. "abomination.svg") or a full URL,
	 * always returns a full URL suitable for <img src>.
	 */
	...
	function notice(msg, type) {
		...
		$el.stop(true, true)
			...
			.text(msg || '');
		setTimeout(function () { $el.fadeOut(250); }, 3200);
	}
	...
	function fillForm(row) {
		...
		// Backend returns filename only for the edit form
	...
	$(document).on('click', '#nw-refresh-btn', function () { loadAll(); });
	$(document).on('click', '.nw-edit-btn',    function () { openModal($(this).data('id')); });

	$(document).on('click', '#nw-modal-close, #nw-cancel-btn', function () { closeModal(); });
	...
	$(document).on('change', '.nw-active-toggle', function () {
		...
		$.post(ajax, {
			...
			all = all.map(function (race) {
				if (String(race.id) === String(id)) { race.is_active = val; }
				return race;
			});
