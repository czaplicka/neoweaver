...
(function () {
	...
	const NW_SFX = (() => {
		...
		return {
			...
			deploy: () => {
				...
				setTimeout(() => beep(660, 'sine', 0.15, 0.25), 120);
			},
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		const campaignsUrl = config.campaignsUrl || '/deployments/';
		...
		const gmStyleLabels = {
			cinematic_heroic: 'Cinematic Heroic',
			harsh_grounded: 'Harsh Grounded',
			fast_tactical: 'Fast Tactical',
		};
		...
		function showStep(idx) {
			...
			if (phase === 'NODE & AGENT BINDING' && !gridsLoaded.nodes) {
				loadNodes();
			}
		...
		function validateStep(idx) {
			...
			const radioFields = {
				'GM PROTOCOL': {
					key: 'gm_style',
					labels: gmStyleLabels,
					msg: 'Select a GM Protocol to continue.',
				},
				'OPERATIVE MODE': {
					key: 'game_mode',
					labels: gameModeLabels,
					msg: 'Select an Operative Mode to continue.',
				},
				'OPERATION SCOPE': {
					key: 'game_length',
					labels: gameLengthLabels,
					msg: 'Select an Operation Scope to continue.',
				},
				'THREAT CALIBRATION': {
					key: 'world_type',
					labels: worldTypeLabels,
					msg: 'Select a Threat Level to continue.',
				},
				'MISSION PRIORITY': {
					key: 'priority',
					labels: priorityLabels,
					msg: 'Select a Mission Priority to continue.',
				},
			};
			...
			if (radioFields[phase]) {
				...
				if (!checked) {
					...
					step.querySelectorAll('input[type="radio"]').forEach((r) =>
						r.addEventListener('change', () => clearFieldError(step), { once: true })
					);
					return false;
				}
				...
				formState[key] = {
					value: checked.value,
					label: labels[checked.value] || checked.value,
				};
		...
		function makeCard(id, name, sub, emoji, selectedId, onSelect) {
			...
			div.addEventListener('click', function () {
				const grid = div.closest('.tw-dynamic-grid');
				if (grid) {
					grid.querySelectorAll('.tw-dyn-card').forEach((c) => c.classList.remove('selected'));
				}
		...
		function loadNodes() {
			...
			if (userId) {
				params.wp_user_id = 'eq.' + encodeURIComponent(String(userId));
			}
		...
		function loadAgents(worldId) {
			...
			const loadToken = ++currentAgentsLoadToken;
			...
			const worldCampaignsUrl = new URL(sbBase + 'cyber_campaign_worlds');
			worldCampaignsUrl.searchParams.set('select', 'campaign_id');
			worldCampaignsUrl.searchParams.set('world_id', 'eq.' + worldId);

			const allAssignedUrl = new URL(sbBase + 'cyber_campaign_characters');
			allAssignedUrl.searchParams.set('select', 'character_id,campaign_id');

			const allCharsUrl = new URL(sbBase + 'cyber_characters');
			allCharsUrl.searchParams.set('select', 'id,name,class_id,race_id,status,cyber_classes(name),cyber_races(name)');
			allCharsUrl.searchParams.set('status', 'neq.STATUS_DEAD');
			allCharsUrl.searchParams.set('order', 'name.asc');

			if (userId) {
				allCharsUrl.searchParams.set('wp_user_id', 'eq.' + String(userId));
			}
			...
			Promise.all([
				fetchJson(worldCampaignsUrl.toString(), { headers }, 10000),
				fetchJson(allAssignedUrl.toString(), { headers }, 10000),
				fetchJson(allCharsUrl.toString(), { headers }, 10000),
			])
				.then(function ([worldCampaigns, allAssigned, allChars]) {
					if (loadToken !== currentAgentsLoadToken) {
						return;
					}
					...
					function renderAgentCard(row, badge) {
						...
						const card = makeCard(
							...
							function (id, name) {
								formState.character_id = { id, name };
								setStatus('', false);
							}
						);
						...
						if (badge) {
							...
							if (strong) {
								strong.insertAdjacentHTML('afterend', badge);
							}
						}
				...
				.catch(function (err) {
					...
					if (err && err.message === 'Request timeout') {
						grid.innerHTML = '<p class="tw-error-msg">Agent request timed out. Try again.</p>';
						return;
					}
		...
		function doSubmit() {
			...
			if (!payload.gm_style) {
				setStatus('ERROR: GM Protocol is required.', true);
				NW_SFX.error();
				return;
			}
			if (!payload.game_mode) {
				setStatus('ERROR: Operative mode is required.', true);
				NW_SFX.error();
				return;
			}
			if (!payload.game_length) {
				setStatus('ERROR: Operation scope is required.', true);
				NW_SFX.error();
				return;
			}
			if (!payload.world_type) {
				setStatus('ERROR: Threat calibration is required.', true);
				NW_SFX.error();
				return;
			}
			if (!payload.priority) {
				setStatus('ERROR: Mission priority is required.', true);
				NW_SFX.error();
				return;
			}
			...
			fetchJson(
				...
				.then(function (json) {
					...
					setTimeout(function () {
						...
						if (json.success) {
							setStatus('// DEPLOYMENT ONLINE: ' + ((json.data && json.data.campaign_id) || ''), false);
							setTimeout(() => {
				...
				.catch(function (err) {
					...
					if (err && err.message === 'Request timeout') {
						setStatus('ERROR: Request timeout. Try again.', true);
					} else {
