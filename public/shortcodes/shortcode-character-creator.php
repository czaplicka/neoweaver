...
if ( ! function_exists( 'neoweaver_register_character_creator_assets' ) ) {
	function neoweaver_register_character_creator_assets(): void {
		if ( ! defined( 'NEOWEAVER_PLUGIN_URL' ) || ! defined( 'NEOWEAVER_PLUGIN_DIR' ) ) {
			return;
		}

		$css_rel = 'assets/css/tw-character-creator-2.css';
		$js_rel  = 'assets/js/tw-character-creator-2.js';
		$css_abs = trailingslashit( NEOWEAVER_PLUGIN_DIR ) . $css_rel;
		$js_abs  = trailingslashit( NEOWEAVER_PLUGIN_DIR ) . $js_rel;

		if ( file_exists( $css_abs ) ) {
			wp_register_style(
				'neoweaver-character-creator',
				trailingslashit( NEOWEAVER_PLUGIN_URL ) . $css_rel,
				[],
		...
		if ( file_exists( $js_abs ) ) {
			wp_register_script(
				'neoweaver-character-creator',
				trailingslashit( NEOWEAVER_PLUGIN_URL ) . $js_rel,
				[],
...
if ( ! function_exists( 'neoweaver_shortcode_character_creator' ) ) {
	function neoweaver_shortcode_character_creator(): string {
		if ( ! is_user_logged_in() ) {
			return '<div class="neoweaver-screen"><div class="tw-error-msg">ACCESS DENIED: Unauthorized Operator.</div></div>';
		}

		wp_enqueue_style( 'neoweaver-character-creator' );
		wp_enqueue_script( 'neoweaver-character-creator' );

		$spinner_css = defined( 'NEOWEAVER_PLUGIN_DIR' ) ? trailingslashit( NEOWEAVER_PLUGIN_DIR ) . 'assets/css/tw-node-spinner.css' : '';
		if ( $spinner_css && file_exists( $spinner_css ) ) {
			wp_enqueue_style(
				'neoweaver-node-spinner',
				trailingslashit( NEOWEAVER_PLUGIN_URL ) . 'assets/css/tw-node-spinner.css',
				[],
		...
		<div id="tw-char-creator-wrapper" data-total-steps="<?php echo esc_attr( $total_steps ); ?>">
			...
			<div class="tw-step active" data-step="1" data-phase="IDENTITY MATRIX">
				...
				<div class="tw-step-error" aria-live="polite">
					<span class="tw-step-error__icon">⚠</span>
					<span class="tw-step-error__msg"></span>
				</div>

				<label class="tw-field-label">
					<span>Agent Designation <span class="tw-required">*</span></span>
					<input type="text" id="tw-char-name" name="character_name" placeholder="e.g. Ghost-7, Mara Voss, The Architect" maxlength="80" required>
				</label>
					...
					<div class="tw-pronoun-options">
						<?php foreach ( $pronoun_options as $value => $label ) : ?>
							<label class="tw-pronoun-option">
								<input type="radio" name="pronouns" id="tw-pronoun-<?php echo esc_attr( sanitize_title( $value ) ); ?>" value="<?php echo esc_attr( $value ); ?>" class="tw-pronoun-radio">
								<span class="tw-pronoun-label"><?php echo esc_html( $label ); ?></span>
							</label>
						<?php endforeach; ?>
					</div>
					<input type="text" id="tw-char-pronouns-custom" name="pronouns_custom" placeholder="e.g. ze/zir, fae/faer" maxlength="40" style="display:none; margin-top:12px;">
				</fieldset>
			...
			<div class="tw-step" data-step="4" data-phase="BIOMETRIC CALIBRATION">
				...
				<p class="tw-question-text">
					...
					<span class="tw-attr-remaining-label">Remaining <span id="tw-attr-remaining"><?php echo esc_html( $attr_pool - count( $attrs ) ); ?></span></span>
				</p>
				<div class="tw-step-error" aria-live="polite">
					<span class="tw-step-error__icon">⚠</span>
					<span class="tw-step-error__msg"></span>
						...
						<div class="tw-attr-row" data-attr="<?php echo esc_attr( $key ); ?>">
							...
							<div class="tw-attr-controls">
								<div class="tw-attr-stepper">
									...
									<input type="number" class="tw-attr-val" name="attr_<?php echo esc_attr( $key ); ?>" id="tw-attr-<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $attr_min ); ?>" min="<?php echo esc_attr( $attr_min ); ?>" max="<?php echo esc_attr( $attr_max ); ?>" readonly aria-label="<?php echo esc_attr( $attr['label'] ); ?> value">
			...
			<div class="tw-step" data-step="10" data-phase="VISUAL SIGNATURE">
				...
				<div class="tw-upload-box" id="tw-avatar-drop">
					<input type="file" id="tw-char-avatar" name="avatar" accept="image/jpeg,image/png,image/webp" style="display:none;">
				...
				<label class="tw-field-label" style="margin-top:24px;">
					...
					<textarea id="tw-char-bio" name="bio" rows="5" placeholder="Write the in-world bio for this Field Agent."></textarea>
				</label>
			...
			<div class="tw-step tw-step--summary" data-step="11" data-phase="SYSTEM REVIEW">
				...
				<div class="tw-summary-grid">
					<div class="tw-summary-row" data-summary-field="character_name"><span class="tw-summary-key">AGENT ID</span><span class="tw-summary-val" id="tw-summary-character-name">—</span><button type="button" class="tw-summary-edit" data-goto="1">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="pronouns"><span class="tw-summary-key">PRONOUNS</span><span class="tw-summary-val" id="tw-summary-pronouns">—</span><button type="button" class="tw-summary-edit" data-goto="1">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="race"><span class="tw-summary-key">RACE</span><span class="tw-summary-val" id="tw-summary-race">—</span><button type="button" class="tw-summary-edit" data-goto="2">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="class"><span class="tw-summary-key">CLASS</span><span class="tw-summary-val" id="tw-summary-class">—</span><button type="button" class="tw-summary-edit" data-goto="3">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="attrs"><span class="tw-summary-key">ATTRIBUTES</span><span class="tw-summary-val" id="tw-summary-attrs">—</span><button type="button" class="tw-summary-edit" data-goto="4">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="skills"><span class="tw-summary-key">SKILLS</span><span class="tw-summary-val" id="tw-summary-skills">—</span><button type="button" class="tw-summary-edit" data-goto="5">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="package"><span class="tw-summary-key">PACKAGE</span><span class="tw-summary-val" id="tw-summary-package">—</span><button type="button" class="tw-summary-edit" data-goto="6">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="origin"><span class="tw-summary-key">DATA ORIGIN</span><span class="tw-summary-val" id="tw-summary-origin">—</span><button type="button" class="tw-summary-edit" data-goto="7">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="operation"><span class="tw-summary-key">PREVIOUS OP</span><span class="tw-summary-val" id="tw-summary-operation">—</span><button type="button" class="tw-summary-edit" data-goto="8">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="crisis"><span class="tw-summary-key">SYNC CRISIS</span><span class="tw-summary-val" id="tw-summary-crisis">—</span><button type="button" class="tw-summary-edit" data-goto="9">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="tag_bundle"><span class="tw-summary-key">BACKSTORY TAGS</span><span class="tw-summary-val" id="tw-summary-tag-bundle">—</span><button type="button" class="tw-summary-edit" data-goto="9">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="avatar"><span class="tw-summary-key">PORTRAIT</span><span class="tw-summary-val" id="tw-summary-avatar">—</span><button type="button" class="tw-summary-edit" data-goto="10">EDIT</button></div>
					<div class="tw-summary-row" data-summary-field="bio"><span class="tw-summary-key">MANUAL BIO</span><span class="tw-summary-val" id="tw-summary-bio">—</span><button type="button" class="tw-summary-edit" data-goto="10">EDIT</button></div>
				</div>

				<div id="tw-char-status-msg" class="tw-char-status" aria-live="polite"></div>
