...
if ( ! function_exists( 'neoweaver_register_character_creator_assets' ) ) {
	function neoweaver_register_character_creator_assets(): void {
		...
		if ( file_exists( $js_abs ) ) {
			...
			// Config used by JS (ajax URL + nonce).
			wp_localize_script(
				'neoweaver-character-creator',
				'twCharCreatorConfig',
				[
...
/**
 * Shortcode renderer: [taleweaver_character_creator]
 */
if ( ! function_exists( 'neoweaver_shortcode_character_creator' ) ) {
	function neoweaver_shortcode_character_creator(): string {
		...
		// Enqueue main CSS/JS if registered.
		...
		// Optional spinner CSS (shared with other NeoWeaver UIs).
		if ( defined( 'NEOWEAVER_PLUGIN_DIR' ) && defined( 'NEOWEAVER_PLUGIN_URL' ) ) {
			$spinner_rel = 'assets/css/tw-node-spinner.css';
			$spinner_abs = trailingslashit( NEOWEAVER_PLUGIN_DIR ) . $spinner_rel;

			if ( file_exists( $spinner_abs ) ) {
				wp_enqueue_style(
					'neoweaver-node-spinner',
					trailingslashit( NEOWEAVER_PLUGIN_URL ) . $spinner_rel,
					[],
					(string) filemtime( $spinner_abs )
				);
			}
		}
		...
		<div id="tw-char-creator-wrapper" data-total-steps="<?php echo esc_attr( $total_steps ); ?>">
			...
			<div class="tw-progress-bar" aria-label="Agent configuration progress">
				...
				<div class="tw-progress-track">
					<div
						class="tw-progress-fill"
						id="tw-char-progress-fill"
				...
				<fieldset class="tw-pronoun-fieldset">
					...
					<div class="tw-pronoun-options">
						<?php foreach ( $pronoun_options as $value => $label ) : ?>
							<label class="tw-pronoun-option" for="tw-pronoun-<?php echo esc_attr( sanitize_title( $value ) ); ?>">
								<input
									type="radio"
									name="pronouns"
			...
			<div class="tw-step" data-step="10" data-phase="VISUAL SIGNATURE">
				...
				<div class="tw-avatar-gallery-block">
					<p class="tw-field-label">
						<span>Or choose from gallery</span>
					</p>
					<div class="tw-avatar-gallery" id="tw-avatar-gallery"></div>
				</div>

				<label class="tw-field-label" for="tw-char-bio" style="margin-top:24px;">
			...
			<div class="tw-step tw-step--summary" data-step="11" data-phase="SYSTEM REVIEW">
				...
				<div class="tw-nav-row">
					<button type="button" class="tw-btn-nav tw-btn-prev">&larr; BACK</button>
					<button type="button" class="tw-btn tw-btn--primary tw-btn-deploy" id="tw-char-submit">
						⌘ SYNCHRONIZE AGENT
					</button>
				</div>
			</div>
