...
if ( ! function_exists( 'tw_list_campaigns_final_v8_modes' ) ) {
	function tw_list_campaigns_final_v8_modes(): string {
		...
		// helper: SVG inline z Lucide CDN (omija problem kolejności ładowania JS)
		$lucide_svg = array(
			'user'         => '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',
			'users'        => '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>',
			'user-round'   => '<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',
			...
			'copy'         => '<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>',
			'x'            => '<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>',
			'log-in'       => '<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg>',
			'anchor'       => '<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22V8"/><path d="M5 12H2a10 10 0 0 0 20 0h-3"/><circle cx="12" cy="5" r="3"/></svg>',
			'syringe'      => '<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="m18 2 4 4"/><path d="m17 7 3-3"/><path d="M19 9 8.7 19.3c-1 1-2.5 1-3.4 0l-.6-.6c-1-1-1-2.5 0-3.4L15 5"/><path d="m9 11 4 4"/><path d="m5 19-3 3"/><path d="m14 4 6 6"/></svg>',
		);
		...
		<div class="tw-campaigns-wrap">
			<div class="tw-campaigns-grid">
				<?php foreach ( $decoded as $c ) : ?>
					...
					if ( ! $world_rel ) {
						$main_btn_icon  = $lucide_svg['anchor'];
						$main_btn_label = 'Anchor Node';
						$main_btn = '<a href="' . esc_url( $nodes_url ) . '" class="tw-btn">' . $main_btn_icon . $main_btn_label . '</a>';
					} elseif ( ! $char_rel ) {
						$main_btn_icon  = $lucide_svg['syringe'];
						$main_btn_label = 'Inject Agent';
						$main_btn = '<a href="' . esc_url( $agents_url ) . '" class="tw-btn">' . $main_btn_icon . $main_btn_label . '</a>';
					} else {
					...
					<div id="campaign-card-<?php echo esc_attr( $c_id_safe ); ?>" class="tw-campaign-card" style="--threat-color:<?php echo esc_attr( $threat_color ); ?>">
						...
						<!-- floating mode badge -->
						<div class="tw-card-mode-badge" title="<?php echo esc_attr( $mode_str ); ?>">
						...
						<div class="tw-card-inner">
							...
							<div class="tw-card-header">
								<div class="tw-card-title"><?php echo $c_name; ?></div>
								<div class="tw-card-world-row">
									...
									<div class="tw-card-badges">
										<?php if ( $threat_data ) : ?>
											<span class="tw-pill" style="color:<?php echo esc_attr( $threat_data['color'] ); ?>;border-color:<?php echo esc_attr( $threat_data['color'] ); ?>44;">
												<?php echo $lucide_svg[ $threat_data['icon'] ] ?? ''; // phpcs:ignore ?>
												<?php echo esc_html( $threat_data['label'] ); ?>
											</span>
										<?php endif; ?>
										<?php if ( $length_data ) : ?>
											<span class="tw-pill tw-pill--length">
												<?php echo $lucide_svg[ $length_data['icon'] ] ?? ''; // phpcs:ignore ?>
							...
							<!-- BODY — 2 kolumny danych -->
							<div class="tw-card-body">

								<!-- Field Agent -->
								<div class="tw-card-cell">
									<div class="tw-card-cell-label">Field Agent</div>
									<?php if ( is_array( $char_rel ) && $operative_name ) : ?>
										<div class="tw-card-cell-value"><?php echo esc_html( $operative_name ); ?></div>
										<?php if ( $operative_race || $operative_class ) : ?>
											<div class="tw-card-cell-sub"><?php echo esc_html( trim( $operative_race . ( $operative_class ? ' · ' . $operative_class : '' ) ) ); ?></div>
										<?php endif; ?>
									<?php else : ?>
										<div class="tw-card-cell-value tw-card-unset">—</div>
									<?php endif; ?>
								</div>

								<!-- Priority -->
								<div class="tw-card-cell">
									<div class="tw-card-cell-label">Priority</div>
									<?php if ( $priority_data ) : ?>
										<div class="tw-card-cell-value">
											<span class="tw-cell-icon">
								...
								<div class="tw-card-body-divider"></div>

								<!-- GM Style -->
								<div class="tw-card-cell">
									<div class="tw-card-cell-label">GM Style</div>
									<?php if ( $gm_data ) : ?>
										<div class="tw-card-cell-value">
											<span class="tw-cell-icon">
								...
								<!-- Mode -->
								<div class="tw-card-cell">
									<div class="tw-card-cell-label">Mode</div>
									<div class="tw-card-cell-value"><?php echo esc_html( $mode_str ); ?></div>
									<?php if ( $is_team && $join_code ) : ?>
										<div class="tw-card-cell-sub" style="color:var(--cyan);letter-spacing:.15em;"><?php echo esc_html( strtoupper( $join_code ) ); ?></div>
									<?php endif; ?>
								</div>
