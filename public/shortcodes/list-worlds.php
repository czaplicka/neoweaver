...
function tw_list_worlds_v14() {
	...
	$world_id_param = isset( $_GET['world_id'] ) ? sanitize_text_field( wp_unslash( $_GET['world_id'] ) ) : '';
	$show_init_banner = ( 'initializing' === $status && ! empty( $world_id_param ) );
		...
		<?php else : ?>
			<div class="tw-world-grid">
				<?php foreach ( $worlds as $w ) : ?>
					...
					$full_desc = ( $p1 || $p2 || $p3 )
						? trim( $p1 . ' ' . $p2 . ' ' . $p3 )
						: trim( $w['world_ai_description'] ?? $w['description'] ?? '' );
					...
					$modal_payload = array(
						...
						'desc'         => $full_desc,
						'magic'        => $m_map[ (int) ( $w['magic'] ?? 0 ) ] ?? ( $w['magic'] ?? '' ),
						'tech'         => $t_map[ (int) ( $w['technology'] ?? 0 ) ] ?? ( $w['technology'] ?? '' ),
						'vibe'         => $v_map[ (int) ( $w['moral'] ?? 0 ) ] ?? ( $w['moral'] ?? '' ),
						'wealth'       => $w_map[ (int) ( $w['wealth'] ?? 0 ) ] ?? ( $w['wealth'] ?? '' ),
						'size'         => $s_map[ (int) ( $w['size'] ?? 0 ) ] ?? ( $w['size'] ?? '' ),
						...
						<div class="tw-card-content">
							<div class="tw-card-top">
								...
								<span class="tw-id-tag">
									...
									&nbsp;|&nbsp;LVL <?php echo (int) ( $w['difficulty'] ?? 0 ); ?>
								</span>
							</div>
