...
class NeoWeaver_Starting_Packages_Admin {
	...
	public function enqueue_assets( string $hook ): void {
		...
		wp_localize_script(
			...
			[
				...
				'nonce'    => wp_create_nonce( 'neoweaver_sp' ),
			]
		);
	}
		...
		<div class="wrap nw-panel" id="nw-sp-panel">
			...
			<div class="nw-table-wrap">
				<table class="nw-table">
					<thead><tr>
						<th>Package Name</th><th>Base Armor</th><th>Slots</th>
						<th>Compat. Tags</th><th>Class IDs</th><th>Player Selectable</th><th>Actions</th>
					</tr></thead>
				...
				<div class="nw-modal">
					...
					<div class="nw-modal-body">
						<form id="nw-sp-form">
							...
							<div class="nw-form-grid">
								<div class="nw-field"><label>Head Slot</label><select id="nw-field-head_item_id" name="head_item_id" class="nw-select nw-item-select"><option value="">— none —</option></select></div>
								<div class="nw-field"><label>Torso Slot</label><select id="nw-field-torso_item_id" name="torso_item_id" class="nw-select nw-item-select"><option value="">— none —</option></select></div>
								<div class="nw-field"><label>Right Hand Slot</label><select id="nw-field-hand_r_item_id" name="hand_r_item_id" class="nw-select nw-item-select"><option value="">— none —</option></select></div>
								<div class="nw-field"><label>Left Hand Slot</label><select id="nw-field-hand_l_item_id" name="hand_l_item_id" class="nw-select nw-item-select"><option value="">— none —</option></select></div>
								<div class="nw-field"><label>Belt Slot</label><select id="nw-field-belt_item_id" name="belt_item_id" class="nw-select nw-item-select"><option value="">— none —</option></select></div>
								<div class="nw-field"><label>Base Armor <span class="nw-hint">(≥ 0)</span></label><input type="number" id="nw-field-base_armor" name="base_armor" min="0" value="0"></div>
							</div>
							<div class="nw-section-label">Card Pools &amp; Lists <span class="nw-hint">(comma-separated → JSON array)</span></div>
							<div class="nw-form-grid">
								<div class="nw-field nw-field-full"><label>Items List</label><input type="text" id="nw-field-items_list" name="items_list" placeholder="item-uuid-1, item-uuid-2"></div>
								<div class="nw-field nw-field-full"><label>Attack Cards Pool</label><input type="text" id="nw-field-attack_cards_pool" name="attack_cards_pool" placeholder="card-id-1, card-id-2"></div>
								<div class="nw-field nw-field-full"><label>Defense Cards Pool</label><input type="text" id="nw-field-defense_cards_pool" name="defense_cards_pool" placeholder="card-id-1, card-id-2"></div>
							</div>
							<div class="nw-section-label">Compatibility <span class="nw-hint">(comma-separated → JSON array)</span></div>
							<div class="nw-form-grid">
								<div class="nw-field nw-field-full"><label>Compatibility Tags</label><input type="text" id="nw-field-compatibility_tags" name="compatibility_tags" placeholder="e.g. melee, urban, stealth"></div>
								<div class="nw-field nw-field-full"><label>Compatible Class IDs</label><input type="text" id="nw-field-compatible_class_ids" name="compatible_class_ids" placeholder="class-uuid-1, class-uuid-2"></div>
							</div>
							...
							<div class="nw-form-grid">
								<div class="nw-field nw-field-center">
									...
									<label class="nw-toggle"><input type="checkbox" id="nw-field-is_player_selectable" name="is_player_selectable"><span class="nw-toggle-slider"></span></label>
								</div>
							</div>
						</form>
					</div>
			...
			<!-- nonce is now także w JS (NW_SP.nonce), ale zostawiamy ten hidden jako fallback -->
	...
	private function supa( string $method, string $endpoint, array $body = [], array $extra = [] ): array {
		...
		$res = wp_remote_request( $this->supabase_url . '/rest/v1/' . ltrim( $endpoint, '/' ), $args );
	...
	public function ajax_toggle(): void {
		...
		wp_send_json_success( [ 'is_player_selectable' => $state ] );
	}
