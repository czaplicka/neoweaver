...
if ( ! class_exists( 'NeoWeaver_Seasons_Admin' ) ) {
	...
	class NeoWeaver_Seasons_Admin {
		...
		private function sanitize_weight( string $key ): int {
			return max( 0, min( 100, (int) ( $_POST[ $key ] ?? 0 ) ) );
		}
		...
		private function build_payload_from_post( string $season_name ): array {
			...
			return array_merge(
				[
					...
					'temp_modifier' => (float) ( $_POST['temp_modifier'] ?? 1.0 ),
		...
		public function ajax_get(): void {
			...
			if ( ! $res['ok'] ) {
				wp_send_json_error( 'Load failed: ' . ( $res['error'] ?? 'Unknown error' ) );
			}
		...
		public function ajax_save(): void {
			...
			if ( $is_edit && '' === $orig_name ) {
				wp_send_json_error( 'Original season name is missing.' );
			}

			if ( $is_edit && $orig_name !== $season_name ) {
				wp_send_json_error( 'Changing season name during edit is disabled. Create a new season instead.' );
			}
			...
			if ( $is_edit ) {
				...
				if ( $exists['ok'] && ! empty( $exists['body'] ) ) {
					wp_send_json_error( 'Season with this name already exists.' );
				}
