...
class NWScenariosAdmin {
	...
	public function enqueue_assets( string $hook ): void {
		...
		wp_enqueue_style( 'nw-admin-scenarios', NW_PLUGIN_URL . 'assets/css/admin/scenarios.css', [], NW_VERSION );
		wp_enqueue_script( 'nw-lucide' );
		wp_enqueue_script( 'nw-admin-scenarios', NW_PLUGIN_URL . 'assets/js/admin/scenarios.js', [ 'nw-lucide' ], NW_VERSION, true );
		wp_localize_script( 'nw-admin-scenarios', 'NW_SCENARIOS', [
	...
	public function render_page(): void {
		echo '<div class="wrap nw-admin-wrap" id="nw-scenarios-app"></div>';
	}

	public function ajax_list(): void {
		check_ajax_referer( 'nw_scenarios_nonce', 'nonce' );
		$resp = nw_supa_get( 'cyber_scenarios?select=*&order=name.asc' );
		wp_send_json( $resp );
	}
	...
	public function ajax_save(): void {
		...
		if ( $id ) {
			$resp = nw_supa_patch( 'cyber_scenarios?id=eq.' . $id, $data );
		} else {
			$resp = nw_supa_post( 'cyber_scenarios', $data );
		}
		wp_send_json( $resp );
	}
	...
	public function ajax_delete(): void {
		...
		$resp = nw_supa_delete( 'cyber_scenarios?id=eq.' . $id );
		wp_send_json( $resp );
	}
}
